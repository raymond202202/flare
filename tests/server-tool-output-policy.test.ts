...
describe('flare server 工具输出治理策略（--tool-output-policy / chat toolOutputPolicy，v0.6.34）', () => {
  ...
  // 网络慢可能超 vitest 默认 5s——与 server.test.ts 同模式放宽 45s
  ...
  it('chat 带合法 toolOutputPolicy（请求覆盖默认）→ 透传流程完整', async () => {
    const msgs = await request({ type: 'chat', sessionId: 's-top-ok', input: '你好', toolOutputPolicy: { maxOutputChars: 400, ellipsis: '[截断]' } }, ['done', 'error'], 45000)
    const last = msgs[msgs.length - 1]
    expect(['done', 'error']).toContain(last.type)
  }, 45000)

  it('chat 带空对象策略 → 等价缺省，事件流完整（零回归）', async () => {
    const msgs = await request({ type: 'chat', sessionId: 's-top-empty', input: '你好', toolOutputPolicy: {} }, ['done', 'error'], 45000)
    const last = msgs[msgs.length - 1]
    expect(['done', 'error']).toContain(last.type)
  }, 45000)

  it('chat 不带 toolOutputPolicy → 应用 server 级默认（事件流完整，不破坏流程）', async () => {
    const msgs = await request({ type: 'chat', sessionId: 's-top-dflt', input: '你好' }, ['done', 'error'], 45000)
