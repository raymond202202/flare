...
响应：`{"type":"usage","stats":{"promptTokens":123,"completionTokens":456,"cacheReadTokens":80,"cacheWriteTokens":0,"estimatedCostUsd":0.0002,"cacheSavedUsd":0.00004,"totalTokens":579,"sessionCount":3,"perModel":[{"model":"deepseek-chat","calls":2,"promptTokens":80,"completionTokens":100,"cacheReadTokens":50,"cacheWriteTokens":0,"totalTokens":180},{"model":"qwen2.5:7b","calls":1,"promptTokens":43,"completionTokens":356,"cacheReadTokens":0,"cacheWriteTokens":0,"totalTokens":399}],"perProvider":[{"provider":"deepseek","calls":2,"promptTokens":80,"completionTokens":100,"cacheReadTokens":50,"cacheWriteTokens":0,"totalTokens":180,"cacheSavedUsd":0.00004},{"provider":"ollama","calls":1,"promptTokens":43,"completionTokens":356,"cacheReadTokens":0,"cacheWriteTokens":0,"totalTokens":399,"cacheSavedUsd":0}]}}`

- 宿主展示用量统计（成本监控）、AI 面板显示 token 消耗时使用
- `perModel`（v0.6.18）：按模型分组的用量分解（`model` / `calls` 调用次数 / `promptTokens` / `completionTokens` / `totalTokens`，按调用次数降序；无模型记录归 `unknown`）——成本核算/用量分布数据源；每项含 `cacheReadTokens`（v0.6.29）与 `cacheSavedUsd`（v0.6.65：本模型缓存节省，同口径单模型差值，无法定价为 0）；v0.6.131 起每项含 `cacheWriteTokens`（本模型缓存写入，与汇总对称）
- `perProvider`（v0.6.136）：按 provider 归并的用量分解（`provider`（detectProvider 推断：ollama 本地 / deepseek / openai / other 线上）/ `calls` / `promptTokens` / `completionTokens` / `cacheReadTokens` / `cacheWriteTokens` / `totalTokens` / `cacheSavedUsd`，按 totalTokens 降序）——混合模式成本收益评估数据源：本地 ollama vs 线上 deepseek/openai/other 分组可见；纯内存归并（perModel 按 detectProvider 分组，无额外查询）；v0.6.138 起 CLI 文本模式「按提供方:」区块 provider 行下显示缓存命中/写入子行（--json 与 server 回包字段不变）
...
- `perModel`（v0.6.52）：本会话按模型分组的用量分解（`model` / `calls` / `promptTokens` /
  ...
  `cacheSavedUsd`；v0.6.131 起每项含 `cacheWriteTokens`）——与 get_usage 的 perModel 对称，宿主面板"本会话用量"可直接显示每个模型的
  缓存命中分布（无需从全局统计里筛）
- `perProvider`（v0.6.136）：本会话按 provider 归并的用量分解——与 get_usage 的 `perProvider`
  同口径（字段一致，按 totalTokens 降序），宿主面板"本会话用量"可直接显示本地 vs 线上分组
