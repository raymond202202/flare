...
- [ ] **P0** 调研：确定方向（多模型 provider 增强）+ 基线实测（tsc 0 错 / 81 全绿）+ 本文件更新
- [ ] **P1** LLM 模型路由（llm.ts）：抽出纯函数 `resolveProviderOptions({ model, baseURL, apiKey })`——模型名含 `:`（Ollama 命名如 `qwen2.5:7b`/`llama3.1:8b`）→ 本地 `http://localhost:11434/v1` + apiKey `ollama`；deepseek/gpt 自动检测保持；claude 明确报错保持；新增 `LLM_BASE_URL` / `LLM_API_KEY` 配置覆盖（config.ts + .env.example）；`createProvider(options?)` 支持传参；导出供测试。新增 tests/llm.test.ts（纯函数，无网络）
